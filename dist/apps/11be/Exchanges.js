...
class Exchanges extends JobQueue {
        constructor(networkID, registryAddress) {
                ...
                this.hotGroups = tokenList => {
                        ...
                        return rc.reduce((result, stat) => {
                                return result && stat === true;
                        });
                };

                this.addrTokenBalance = tokenSymbol => addr => {
                ...
                this.addrStates = (tokenSymbol, bucketNo) => addr => {
                        if (typeof this.CUE.Token[tokenSymbol] === 'undefined') throw new Error(`Token ${tokenSymbol} is not part of current hot group`);
                ...
                this.closerStates = tokenSymbol => addr => {
                        if (typeof this.CUE.Token[tokenSymbol] === 'undefined') throw new Error(`Token ${tokenSymbol} is not part of current hot group`);

                        let [since, seen, cc, times, done, reward] = this.CUE.Exchange[tokenSymbol].closerStat({ from: addr }).map(i => {
                                return this.web3.toDecimal(i);
                        });
                ...
                Object.keys(allConditions).map(f => {
                        if (typeof this[f] === 'undefined') this[f] = allConditions[f];
                });
        }
