...
class MarketStat extends Exchanges {
	...
	marketSummary = bucket =>
	{
	        let [ bucketSold, 
		      ...
		      bucketSupply, 
		      currentPrice, 
		      minPerStock ] = this.CUE.Exchange[this.tokenSymbol].marketSummary(bucket).map((u) => { return this.web3.toDecimal(u) });
	...
	{
	        let [ethBalance, extraReward] = this.CUE.Exchange[this.tokenSymbol].marketFinance(bucket).map((u) => { return this.web3.toDecimal(this.web3.fromWei(u, 'ether')); });
	...
	bucketLimits = bucket =>
	{
	        let [maxListing, peakListing, slotReused, slotSettled] = this.CUE.Database[this.tokenSymbol].bucketLimits(bucket).map((u) => { return this.web3.toDecimal(u)});
