...
class CastIron extends Exchanges {
	...
	closerStates = addr => {
		let [since, seen, cc, times, done, reward] = this.B.closerStat({from: addr}).map((i) => { return this.web3.toDecimal(i); });
	...
	giveTokens = Q => (issuer, tokenUnits) => statObj => {
	        if ( 
			this.B.paused() == false 
		     && this.T.balanceOf(issuer).gt(tokenUnits) 
		     && this.T.balanceOf(statObj.addr).lt(1000000000000)
		) {
	...
	depositToken = Q => tokenUnits => statObj => {
	        if ( 
			this.B.paused() == false 
		     && this.T.balanceOf(statObj.addr).gte(tokenUnits) 
		     && statObj.stock == 0 
		     && statObj.owe == false 
	...
	giveStakes = Q => (issuer, tokenUnits) => cstatObj => {
	...
	depositStake = Q => tokenUnits => cstatObj => {
	...
	estimateClosing = (bucketNo, earning) => addr => {
	...
	{
		if (
	                cestObj.cost <= this.maxGasUsage    // max gas spending 
	...
	changePrice = Q => (issuer, newPrice) => {
		if (
			this.B.paused() == true
		     && this.B.changePrice.estimateGas(newPrice, {from: issuer}) < 900000
		) {
